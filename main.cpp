...
*************************************************************************************
*                                                                                   *
*               MineCraft Alpha Server Launcher C++ Version 1.0 * By:OFFMN *
*               This software complies with MIT. * Please do not use it for
*commercial purposes.                       * The software only supports
*Windows.                                 *
*                                                                                   *
*************************************************************************************
*/
...
std::string Stupid_proof_stupid_proof_and_uninformed_design_001_select =
    ""; // 用于存储用户选择
  ...
  while (std::getline(file, line)) // 读取剩余行
  {
    if (line.find("RAMmax") != std::string::npos) {
      ...
      if (serverini_jarname == "server.jar") {
        std::cout
            << "请替换 server.ini 中的 JARname 为实际的服务器JAR文件名。\n";
      } else if (serverini_jarname.empty()) {
        std::cerr
            << "JARname 不能为空，请在 server.ini 中设置正确的 JAR 文件名。\n";
      }
...
void line_about() {
  std::cout << "<-[MineCraft Alpha Server Launcher C++]-{Version: " << VERSION
            << "}----------------=+By:OFFMN+=------->\n";
}
void download_jar(const std::string &url) // 移除多余的逗号
{
  ...
  // 下载文件
  HRESULT hr =
      URLDownloadToFileA(NULL, url.c_str(), file_path.c_str(), 0, NULL);
...
void frp_install() { std::cout << "no you can't\n"; }
void java_install() {
  std::cout << "\n[法律声明]\n";
  ...
  std::string java_url =
      ...
      "jdk8u292-b10/OpenJDK8U-jdk_x64_windows_hotspot_8u292b10.msi";
...
void mcserver_with_gui() {
  ...
  read_ini_file();
  std::string command = "java -Xmx" + std::to_string(serverini_rammax) +
                        "M -Xms" + std::to_string(serverini_rammin) +
                        "M -jar " + serverini_jarname;
...
int main() {
  ...
  if (Stupid_proof_stupid_proof_and_uninformed_design_001_select != "y" &&
      Stupid_proof_stupid_proof_and_uninformed_design_001_select != "Y") {
  ...
  while (true) {
    ...
    case MENU_DISCLAIMER:
      ...
      std::cout << "如果您不确定，请勿使用本工具。" << std::endl;
      std::cout
          << "请注意，使用本程序可能会导致数据丢失或其他问题，请谨慎操作。" << std::endl;
      std::cout << "开发者不对因使用本程序而导致的任何损失或损害负责。" << std::endl;
      line_tips();
      break; // 有效选项，继续执行
    default:
      std::cerr << "无效的选项，请重新选择。" << std::endl;
      continue;
    }
  }
}